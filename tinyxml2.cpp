...
namespace __TinyXml2_AutoFree {
    struct FREE_MEMORY_DATA
    {
        char* pstrA;            // 释放的内存
        wchar_t* pstrW;         // 释放的内存
        ULONGLONG joinTimer;    // 加入释放队列时的事件
        FREE_MEMORY_DATA() :pstrA(0), pstrW(0), joinTimer(0) { }
        FREE_MEMORY_DATA(char* A, wchar_t* W)
        {
            this->pstrA = A;
            this->pstrW = W;
            this->joinTimer = GetTickCount_d();
        }
    private:
        inline static ULONGLONG GetTickCount_d()
        {
#if _MSC_VER < 1201
            ...
            if (hModule)
            {
                fnGetTickCount64 fun = (fnGetTickCount64)GetProcAddress(hModule, "GetTickCount64");
                if (fun) return fun();
            }
            return GetTickCount();
#else
    ...
    static std::queue<FREE_MEMORY_DATA> s_queueFree;
    static LPCRITICAL_SECTION s_free_xkq;

    //static void CALLBACK TimerProc_free_pstr(HWND hWnd, UINT message, UINT_PTR id, DWORD dwTimer)
    static DWORD CALLBACK ThreadProc_free_pstr(LPVOID lpParam)
    {
        while (s_free_xkq)
        {
            // 因为加入的是按顺序来加的, 所以第一个加入的时间肯定比第二个要早
            // 如果第一个小于5秒就可以退出循环, 否则一直循环判断第一个, 大于10秒就弹出

            Sleep(1000);    // 线程内, 调用这个延时就可以
            EnterCriticalSection(s_free_xkq);   // 进入许可区
            while (s_queueFree.size())
            {
                FREE_MEMORY_DATA& data = s_queueFree.front();
#ifdef _DEBUG
                const ULONGLONG freeTime = 60 * 60 * 1000;  // 调试版60分钟后释放
#else
                const ULONGLONG freeTime = 20000;           // 编译版20秒后释放
#endif
                if (GetTickCount64() - data.joinTimer < freeTime)break;
                if (data.pstrA) delete[] data.pstrA;
                if (data.pstrW) delete[] data.pstrW;
                s_queueFree.pop();
            }
            LeaveCriticalSection(s_free_xkq);   // 退出许可区
        }
        if (s_free_xkq)DeleteCriticalSection(s_free_xkq);
        if (s_free_xkq)delete s_free_xkq;
        return 0;
    }


    void push_free_memory(char* A, wchar_t* W)
    {
        if (!s_free_xkq)
        {
        ...
        EnterCriticalSection(s_free_xkq);   // 进入许可区
        s_queueFree.push(FREE_MEMORY_DATA(A, W));
        LeaveCriticalSection(s_free_xkq);   // 退出许可区
    }
