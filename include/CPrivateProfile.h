...
public:
    inline const wchar_t* GetPathW() { return m_file.c_str(); }
    inline const _str GetPathA() { return m_file.w2a(); }
    ...
    inline bool write(const wchar_t* AppName, const wchar_t* KeyName = 0, const wchar_t* String = 0)
    {
        if (m_file.empty())return 0;
        return WritePrivateProfileStringW(AppName, KeyName, String, m_file.c_str());
    }
    // 写配置项A版
    // AppName = 配置节名
    // KeyName = 配置项名 为空则删除这个小节的所有设置项
    // String  = 写入内容 为空则删除这个项现有的字串
    inline bool write(const char* AppName, const char* KeyName = 0, const char* String = 0)
    {
        if (m_file.empty())return 0;
        return WritePrivateProfileStringA(AppName, KeyName, String, m_file.w2a().c_str());
    }
    ...
    // KeyName = 配置项名
    // String  = 写入内容
    inline bool write(const wchar_t* AppName, const wchar_t* KeyName, LONGLONG String)
    {
        if (m_file.empty())return 0;
        wchar_t buffer[50] = { 0 };
        swprintf_api(buffer, 50, L"%lld", String);
        return WritePrivateProfileStringW(AppName, KeyName, buffer, m_file.c_str());
    }
