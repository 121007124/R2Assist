...
#define EM_SHOWBALLOONTIP   (ECM_FIRST + 3) 
    ...
    inline int GetFirstVisibleLine() {
    ...
    inline HLOCAL GetHandle(bool* isUnicode) {
    ...
    // 获取用于编辑或丰富编辑控件的密码字符
    inline wchar_t GetPasswordChar() {
    ...
    inline EDITWORDBREAKPROC GetWordBreakProc() {
    ...
    // 隐藏与编辑控件关联的所有气球提示
    inline bool HideBalloonTip() {
        return Send(EM_HIDEBALLOONTIP, 0, 0);
    }

    // 限制可以输入到编辑控件中的文本的长度
    inline void LimitText(int cchMax) {
    ...
    inline void ReplaceSel(LPCWSTR lpszReplace, bool canUndone = false) {
        Send(EM_REPLACESEL, (WPARAM)canUndone, (LPARAM)lpszReplace);
    }

    // 在多行编辑或丰富编辑控件中垂直滚动文本
    // dv = 垂直滚值, dh = 水平滚动的值
    inline void Scroll(int dv, int dh) {
        Edit_Scroll(m_hWnd, dv, dh);
    }

    // 在编辑或丰富编辑控件中将插入符滚动到视图中
    inline bool ScrollCaret() {
        return Edit_ScrollCaret(m_hWnd);
    }

    // 设置编辑控件中插入符号位置的从零开始的索引
    // newCaretIndex = 字符索引
    inline bool SetCaretIndex(int newCaretIndex) {
    ...
    // dwMask = 该值指定要受影响的样式
    inline bool SetExtendedStyle(DWORD dwStyle, DWORD dwMask) {
        return Send(EM_SETEXTENDEDSTYLE, dwMask, dwStyle);
    }
    ...
    inline void SetPasswordChar(wchar_t ch) {
    ...
    inline bool SetReadOnly(bool fReadOnly) {
    ...
    inline void SetRectNoPaint(const RECT& rc) {
    ...
    inline void SetTabStops(int cTabs, int* lpTabs) {
        Edit_SetTabStops(m_hWnd, cTabs, lpTabs);
    }

    // 设置编辑控件的文本
    inline bool SetText(UINT uValue, BOOL bSigned) {
    ...
    // 设置编辑控件的文本
    inline bool SetText(LPCWSTR lpsz) {
        return SetWindowTextW(lpsz);
    }

    // 设置编辑控件的文本
    inline bool SetTextA(LPCSTR lpsz) {
    ...
    inline bool ShowBalloonTip(PEDITBALLOONTIP peditballoontip) {
        return Send(EM_SHOWBALLOONTIP, 0, (LPARAM)peditballoontip);
    }
