...
inline const char* _Unpublished_getPinyinByCode(LONG code)
{
    switch (code)
    {
    case 6325:
...
inline void GetPinyin(const char* szChinese, char pinyinBuf[], const LONG maxBufLen, LONG* bufLen, const LONG mode)
{
    *bufLen = 0;
    UCHAR ucHigh, ucLow;
    LONG code, i, j;
    const LONG chineseLen = strlen(szChinese);
    for (i = 0; i < chineseLen; ++i)
    {
        ...
        ucHigh = (UCHAR)szChinese[i];
        ucLow = (UCHAR)szChinese[++i];
        ...
        const char* pBuf = _Unpublished_getPinyinByCode(code);
        for (j = 0; j < (LONG)strlen(pBuf) && (*bufLen) < maxBufLen; ++j)
        {
            char cc = pBuf[j];
            switch (mode)
            {
