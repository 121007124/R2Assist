...
private:
    ...
    inline static int __debug(str_base* pstr, size_t bufferSize, const_pointer format, va_list list)
    {
        ...
        if ( bufferSize == 0 )
        {
            int r = tstr_vscprintf(format, list) + 1;
            isC = true;
            bufferSize = r > 1 ? r : 260;
        }
        ...
        if ( !isC && r == -1 )
        {
            r = tstr_vscprintf(format, list) + 1;
            if ( r > 1 )
            {
                buf.resize(r);
                buffer = buf.data();
                r = tstr_vsnprintf(buffer, r, format, list);
            }
        }
        while (r == -1)
        {
...
#pragma region _append_assign
...
#endif
    // 赋值, len = 0则以\0结尾
    inline str_base& assign(const_pointer _ptr, const size_t len = 0)
    {
        setstr(_ptr, len);
        return *this;
    }
    // 赋值为一段重复字符的文本, _count = 重复数量, _ch = 字符
    inline str_base& assign(const size_t _count, value_type _ch)
    {
        if ( _count == 0 )
        {
            clear();
    ...
    inline str_base& assign(const size_t _count, const_pointer _ptr, size_t strLen)
    {
        if ( _count == 0 )
        {
            clear();
...
#pragma region text_resize_format_cmp_toup_tolo
    ...
    // 分配一块内存, 内部调用 resize 实现
    inline pointer apply(size_t newsize, value_type _ch = 0)
    {
        resize(newsize, _ch);
        return m_ptr ? m_ptr->pData : 0;
    }
    ...
    inline void resize(size_t newsize, value_type _ch = 0)
    {
        ...
        if ( newsize <= oldSize )
        {
            // 新尺寸小于等于原来的尺寸, 从新尺寸的位置到旧尺寸的位置填充数据
            for ( size_t i = newsize; i < oldSize; i++ )
                m_ptr->pData[i] = _ch;    // 超出部分用指定字符填充
        }
        else
        {
            // 新尺寸大于旧尺寸, 那就是前面已经赋值过了, 这里就不处理
        }
...
#pragma region _find_erase_replace_left_right_
    ...
    {
        return erase(m_ptr->pData, _off, _count);
    }
    // 删除子文本, 结果保存到本字符串里
    // _off    = 起始位置
    // count= 删除字符数,多字节单位为字节,Unicode为字符, 0=从起始位置删除到结束
    ...
    {
        return left(_string.c_str(), _off);
    }
    // 取文本左边,取指定字符串左边的文本串
    // _string = 取这个字符串左边的文本串,返回的文本串不包含该字符串
    // _off = 起始查找参数1的位置
    ...
    str_base right(const str_base& s, bool b = false, const size_t _off = size_t(-1))const
    {
        ...
        return right(m_ptr->len - pos - (b ? 0 : s.size()));
    }
    // 取文本右边,取指定字符串右边的文本串
    // s    = 取这个字符串右边的文本串,返回的文本串不包含该字符串
    // b    = 是否保留被查找字符串
    ...
    str_base right(const_pointer s, bool b = false, const size_t _off = size_t(-1))const
    {
        ...
        //int count = str_base::find(s);
        if (pos == str_base::npos)return str_base();
        return right(m_ptr->len - pos - (b ? 0 : len));
    }
    ...
    str_base substr(size_t pos, size_t count = size_t(-1))const
    {
        ...
        size_t newLen = count;
        return str_base(m_ptr->pData + pos, newLen);
    }
    ...
    str_base substr(const_pointer s1, const_pointer s2, const size_t _off = 0, bool b = false)const
    {
        ...
        if (b)
        {
            return substr(pos1, pos2 - pos1 + len2);
        }

        pos1 += (int)len1;        // 不保留前后字符串, 找到位置 + 前面字符串长度
        return substr(pos1, (size_t)pos2 - (size_t)pos1);
    }
    ...
    str_base RTrim(const value_type _ch = ' ')const
    {
        ...
        return str_base(m_ptr->pData, nLen);
    }
#pragma endregion
...
public:
    ...
    // 附加指针, 只能附加本类.data(), GetPtr() 返回的值, 附加其他值会崩溃
    ...
    // 申请指定字符串缓冲区, 会增加一个结束标志,长度只需要输入字符串实际长度就可以了, 这个方法不会填充返回的内存
    // 不使用时必须调用本对象的 free() 释放
    inline static pointer alloc(size_t size)
    {
        return tstr_alloc<value_type>(size);
    }
    
    // 申请指定字符串缓冲区, 会增加一个结束标志,长度只需要输入字符串实际长度就可以了
    // 不使用时必须调用本对象的 free() 释放
    inline static pointer alloc(size_t size, value_type fill)
    {
...
#pragma region __inside_static_private
    ...
    pointer realloc(size_t size, bool isClear = false)
    {
        ...
        {
            pData = (LPSTRMEMORYDATA)alloc(len, 0);
            pData->Addref = 1;
...
public:
    ...
    {
        return tolower(_string.c_str(), _string.size(), _off, _count);
    }
    ...
    {
        return toupper(_string.c_str(), _string.size(), _off, _count);
    }
    // 把当前字符串转成大写, _off=起始位置, _count=转换的字符数, 返回替换的字符数
    // _string = 被转换的字符串
    // _strlen = 被转换字符串长度, 如果此值为0, 那_string必须以\0为结尾
    ...
    static size_t strtok(const_pointer str, strArr<value_type>* arr, const_pointer _substr, const size_t _count = size_t(-1))
    {
        ...
        pointer ptr = tstr_tok(p->str, _substr, &next_token);
        ...
        while (ptr)
        {
            ...
            ptr = tstr_tok(NULL, _substr, &next_token);
        }
        p->count = n;
        if (!arr)
        {
    ...
    {
        return left(str.c_str(), _string.c_str(), _off);
    }
    // 取文本左边,取指定字符串左边的文本串
    // str      = 源字符串
    // _string  = 取这个字符串左边的文本串,返回的文本串不包含该字符串
    ...
    {
        return right(str.c_str(), s.c_str(), b, _off);
    }
    // 取文本右边,取指定字符串右边的文本串
    // s    = 取这个字符串右边的文本串,返回的文本串不包含该字符串
    // b    = 是否保留被查找字符串
    ...
    inline static str_base right(const_pointer str, const_pointer s, bool b = false, const size_t _off = size_t(-1))
    {
        ...
        //int count = str_base::find(s);
        if (pos == str_base::npos)return str_base();
        return right(str, len1 - pos - (b ? 0 : len2));
    }
